...
| `KAIROS_AI_PROVIDER` | API | `openrouter` | The active AI provider to use. |
...
| `KAIROS_AI_PROVIDER_FALLBACK_ORDER` | API | `ai.openrouter,ai.commandcode,ai.ollama,ai.codex,ai.openai,ai.deepseek,ai.9router,ai.gemini,ai.claude` | Provider router fallback order. OpenRouter is default cloud provider. |
...
auto/manual selection and fallback order. OpenRouter is the default cloud
provider (set `OPENROUTER_API_KEY`). Ollama and Command Code are also
functional. OpenAI, Gemini, and Claude are metadata-only stubs with no OAuth,
